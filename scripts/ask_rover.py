#!/usr/bin/env python3
"""MSE-6 operator Q&A dry-run probe.

Answers a single natural-language QUESTION via the deliberative LLM gateway's
conversational ``answer_query`` path — WITHOUT issuing any motor command — so
the cloud-Claude / local-fallback brain can be asked questions on the rover
even when the ESP32 / drivetrain is detached. This is the conversational
sibling of ``scripts/translate_mission.py`` (which translates a navigation
command into a ``GoalVector``); this script returns free-text prose instead.
...
The query passes the gateway's prompt-injection filter before any third-party
egress, identical to the mission path — an ``InjectionRejected`` (a
``ValueError`` subclass) exits with the runtime-error code.

Config resolution mirrors ``translate_mission.py`` via
...
Examples::
    ...
    MOUSEDROID_CONFIG=/etc/mousedroid/jetson_production.yaml \\
        python scripts/ask_rover.py --query "what is your battery policy?"

    # Dev box with an explicit overlay:
    python scripts/ask_rover.py \\
        --config config/jetson_production.yaml --query "who are you?"

Exit codes:

* ``0`` — query answered (prints the answer + tier/degraded state).
* ``1`` — runtime / gateway failure (incl. injection rejection).
...
# the answer print on stdout clean for piping. (Mirrors translate_mission.py.)
...
from mousedroid.llm_gateway.protocol import (  # noqa: E402
    LLMGatewayProtocol,
    QueryCapableLLMProtocol,
)
...
def _parse_args(argv: list[str] | None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--query",
        required=True,
        help="Natural-language question for the rover to answer.",
    )
    parser.add_argument(
        "--config",
        action="append",
...
def _describe_tier(gateway: object) -> str:
    """Best-effort label for which tier served the answer.
    ...
    part of :class:`LLMGatewayProtocol`, so every read is guarded. Mirrors
    ``translate_mission.py._describe_tier``.
...
async def _run(query: str, gateway: LLMGatewayProtocol) -> int:
    """Start the gateway, answer one query, print the result, stop.

    ``stop()`` is guaranteed even if ``start()`` or ``answer_query()`` raises —
    both are inside the single ``try`` whose ``finally`` calls stop.
    ...
    """
    if not isinstance(gateway, QueryCapableLLMProtocol):
        # Should never happen — every backend built by the factory implements
        # answer_query — but fail loudly rather than silently if a future
        # backend forgets the capability.
        _log.error("ask_rover_gateway_not_query_capable", gateway=type(gateway).__name__)
        return _EXIT_RUNTIME_ERROR

    try:
        await gateway.start()
        answer = await gateway.answer_query(query)
        tier = _describe_tier(gateway)  # capture before stop() may clear degraded state
    finally:
        await gateway.stop()

    print(f"query={query!r} tier={tier} answer={answer!r}")
    return _EXIT_OK
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        _log.exception("ask_rover_build_error")
        return _EXIT_RUNTIME_ERROR

    try:
        return asyncio.run(_run(args.query, gateway))
    except Exception:
        _log.exception("ask_rover_runtime_error")
        return _EXIT_RUNTIME_ERROR
