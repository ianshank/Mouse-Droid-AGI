...
so it is deliberately not claimed as enforced here. Beyond lint/type/test, CI runs `config-compat` (schema-drift), `actionlint` (workflow lint), and a cyclomatic-complexity gate (`ruff C901`, max 15; [ADR-014](docs/architecture/ADR-014-cyclomatic-complexity-gate.md)). Full strategy: [`docs/testing.md`](docs/testing.md) and [`docs/planning/VALIDATION_REPORT.md`](docs/planning/VALIDATION_REPORT.md).
