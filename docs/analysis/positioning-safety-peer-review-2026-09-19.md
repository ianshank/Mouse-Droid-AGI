...
| P7 | "TensorRT engines must be rebuilt per GPU and TensorRT version" | **CONFIRMED, AND THE REVIEW UNDERSTATES IT — this is a live defect today** | See D-7. Not a currency observation: it fires on a GPU swap or a base-image bump regardless of whether anyone ever upgrades JetPack |
...
| **D-16** | **Low** | **`cfg.loop.max_miss_pct` is phantom config, and the CI test named for deadline-miss does not measure one.** `LoopConfig` has no `max_miss_pct` field, so `tests/hardware/test_e2e_sense_plan_act.py`'s `getattr` fallback always wins. Separately `tests/integration/test_e2e_5sec_run.py::test_deadline_miss_rate_below_threshold` — which **does** run in CI — computes a p90 over 10 mock ticks against a ~3333 ms budget and computes **no miss rate at all**. Same class as S-10 | `tests/hardware/`, `tests/integration/` |
...
| "Dual-track JetPack 7.2 now; RT kernel serves the 30 Hz story" | Drop the RT-kernel argument — nothing in the design depends on kernel preemption. Keep the *testable* residue: promote the CUDA/TensorRT/L4T triple to a schema contract (the `esp32.command_set` precedent, which has a **raising** validator) and fix D-7, both of which are worth doing whether or not you ever upgrade |
