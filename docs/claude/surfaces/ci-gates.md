...
5. **Stage 2** — `typecheck`: `mypy --strict` on `src/` and `tools/claude_hooks/` (matrixed).
   Installs `.[dev,telemetry,mlflow]` so one dependency set covers every mypy run: the
   `telemetry` extra supplies the real aiohttp/Pillow types, and `mlflow-skinny`'s own type
   information is what makes the mlflow-facing `cast`s checkable here rather than in
   `mlflow-extras`, which installs no `telemetry` extra.
...
9. **Stage 3c** — `local-gates`: dedicated `--cov=tools/claude_hooks` coverage gate plus the
   ...
   `doc_hygiene.py --strict`, ratchet budgets (`--strict`: a ceiling breach fails the job; an
   approaching-budget warning still exits 0), hardcoded-value gate, subsystem-boundary gate).
...
13. **Stage 4b2** *(advisory)* — `mlflow-extras`: mlflow-touching test suite with the
    `[mlflow]` extra installed (F-034) — the first job to ever install it in CI. Runtime
    exercise only: it carries no mypy step, because `.[dev,mlflow]` brings no `telemetry`
    extra, so aiohttp is absent and `mypy --strict` reports `untyped-decorator` on the
    `@web.middleware` functions. The mlflow typecheck lives in the blocking `typecheck` job.
...
16. **Stage 5** — `security`: `pip-audit --skip-editable` — blocking since 2026-09-16 (60-day
    advisory window opened 2026-07-25; promoted on zero findings).
