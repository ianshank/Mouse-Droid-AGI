...
| Sensors | `src/mousedroid/sensing/manager.py` | Vision (256-d IMX500), 2D LiDAR (36-bin), ultrasonic (1-d), motor (4-d), audio (192-d) |
| Robot geometry | `RobotConfig` in `config/schema.py:1146` | mecanum, wheelbase 0.20 m, track 0.20 m, wheel r=0.042 m |
| VLA inference | `src/mousedroid/vla/policy.py` | ONNX Runtime + TensorRT, weights pulled from HF `ianshank/mousedroid-weights` |
| Jetson runtime | `JetsonConfig` in `config/schema.py:464`, `Dockerfile.jetson` | CUDA 12.6, TensorRT, FP16, 0.5 GPU mem fraction |
...
|---|---|
| `wheel_base=0.20`, `track_width=0.20`, `wheel_type="mecanum"`, `wheel_radius=0.042` | `mse6_rover.urdf` generated by `sim/isaac_lab/robot_usd.py` from config |
| 3-DoF action `(vx, vy, ω)` with `max_velocity_mps=0.5`, `max_omega_rads=2.0` | `HolonomicController` driving 4 mecanum joints; action-space limits clamp to `ESP32Config` |
...
> n-gon wheels for mecanum and tank drives. We must validate the holonomic
> controller against the Wave Rover's real velocity-tracking response curve
> before committing to it. Mitigation: log real wheel encoder traces in Phase 0
> and fit motor dynamics into the URDF actuator model.
...
| FHL-LD19 2D LiDAR → 36 angular bins | `RTX Lidar` configured as 2D scan; bin to 36 angles in env wrapper | Match scan FoV/range from `LidarConfig` |
...
All five outputs are concatenated into the same 256-d obs vector
(`obs_dim=256`) that `world_model/encoder.py` already consumes — no encoder
changes needed.
...
The current `DomainRandomizationConfig` (lines 267–319 of `schema.py`) already
defines ranges for vision brightness/contrast, gaussian noise, motor latency,
chassis dynamics. We **keep the same Pydantic schema** and add a
...
| **2 — Sensor parity** | 1 week | RTX LiDAR → 36-bin scan, depth/RGB camera → 256-d feature, ultrasonic ray-cast — all flowing into the same `MouseDroidObservationBundle` | Property tests in `tests/property/` show obs distributions overlap with mock-data baselines within KL < 0.5 |
...
| R2 | Isaac Lab requires Python 3.11 + Linux x86_64 + dGPU; team workstations may not all qualify | Medium | Medium | Stand up a single shared training rig + CI runner; dev workflow stays on existing mock path |
