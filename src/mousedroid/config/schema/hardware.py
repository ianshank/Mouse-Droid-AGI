...
class JetsonConfig(StrictBaseModel):
    ...
    tensorrt_cache_dir: Path = Field(
        ...
        description=(
            ...
            "treated as a MISS and rebuilt, never loaded. This path is "
            "mounted from the named Docker volume `mousedroid_tensorrt_cache` "
            "by docker-compose.jetson.yml (F-051), so the cache still "
            "outlives the image and survives --force-recreate, but it is no "
            "longer a bind-mount of a host path under /opt/mousedroid. The "
            "0700 mode is deliberately NOT "
