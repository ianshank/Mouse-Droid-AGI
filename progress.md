...
**Features worked:** F-015 (secret-scan gate, WS-0.4), F-016 (truth reconciliation, WS-1), F-017 (host-env durability, WS-3.1) — rev. B validation-first plan, PR #151.
**Status changes:** F-015, F-016, F-017 → done (branch provenance; swap to squash SHA post-merge).
