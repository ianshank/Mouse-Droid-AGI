...
- [ ] 4.1 Read full AlayaWorld PDF sections on bounded context design and
...
- [ ] 4.2 Document MouseDroid's current world-model state representation and
      existing memory scheme (baseline).
      → "Baseline" section of
      `docs/superpowers/specs/2026-07-23-alayaworld-memory-distill-design.md`.
- [ ] 4.3 Design bounded-context memory manager interface adapted to droid state
      representation (recurrent latent `(h, z)`, not video frames).
      → `LatentContextProtocol` (design spec §D1 + ADR-015).
- [ ] 4.4 Implement memory manager as an ablation-switchable module (old vs new
...
- [ ] 4.5 Implement corrupted-history rollout generator for training data
...
- [ ] 4.6 Implement residual-prediction training objective.
...
- [ ] 4.7 Run comparison: baseline memory/training vs new memory/training on the
...
- [ ] 4.8 Run distillation feasibility spike on target edge hardware (Jetson)
...
- [ ] 4.9 Write spike report with go/no-go recommendation.
      → `docs/analysis/alayaworld-distillation-spike.md` (provisional/conditional
      pending the Jetson run).
- [ ] 4.10 Update `docs/related-work.md` with AlayaWorld citation, explicitly
