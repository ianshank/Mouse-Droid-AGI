...
|---|---|---|---|---|
| `mouse-droid-mlflow-sqlite-tracking` | in progress | F-034 | — | `src/mousedroid/config/schema/telemetry.py` (`ExperimentLoggerConfig.tracking_uri`), `pyproject.toml` (`[mlflow]` extra), `.github/workflows/ci.yml` (`mlflow-extras` job), `docs/architecture/c4-experiment-logger.md`, `docs/runbooks/mlflow-local-ui.md`, `scripts/validations/F-034.sh`, `features.yaml` (F-034) |
