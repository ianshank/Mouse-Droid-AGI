...
|---|---|---|---|---|
| `mouse-droid-jetson-onnx-delivery` | proposed | F-050, F-051 | — | `openspec/changes/mouse-droid-jetson-onnx-delivery/peer-review.md` (rev-A review), `docs/architecture/ADR-008-world-model-onnx-engine.md` (amended by the planned ADR-019), `src/mousedroid/common/onnx_session.py`, `src/mousedroid/world_model/dual_stream_rssm_onnx.py`, `src/mousedroid/factory/world_model.py` (`build_world_model` metrics seam), `src/mousedroid/config/schema/world_model.py`, `config/prometheus/alerts.yml` (`WorldModelObserveStepLatencyHigh`), `tests/performance/test_observe_step_budget.py`, `scripts/benchmark_latency.py`, `scripts/deploy_remote.sh`, `docker-compose.jetson.yml`, `deployments/jetson-image.json` |
