...
|---|---|---|---|---|
| `mouse-droid-gcp-observability-wiring` | in progress | F-032 | — | `src/mousedroid/cloud/protocol.py` (`CloudFirestoreSyncProtocol`, widened `CloudLoggingSinkProtocol`), `src/mousedroid/factory.py` (`build_cloud_logging_sink`, `build_cloud_metrics_exporter`, `build_cloud_firestore_sync`), `src/mousedroid/orchestrator/orchestrator.py`, `src/mousedroid/main.py`, `scripts/validations/F-032.sh`, `features.yaml` (F-032) |
